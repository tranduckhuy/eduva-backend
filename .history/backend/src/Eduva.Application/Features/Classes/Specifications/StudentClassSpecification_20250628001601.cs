...
namespace Eduva.Application.Features.Classes.Specifications
{
    public class StudentClassSpecification : ISpecification<StudentClass>
    {
        ...
        public StudentClassSpecification(StudentClassSpecParam param, Guid? classId = null)
        {
            ApplyFilterCriteria(param, classId);
            AddIncludes();
            ApplySorting(param);
            ApplyPaging(param);
        }

        private void ApplyFilterCriteria(StudentClassSpecParam param, Guid? classId)
        {
            // Start with the base criteria
            Criteria = sc => true;

            if (classId.HasValue)
            {
                Criteria = CombineWithAnd(Criteria, sc => sc.ClassId == classId.Value);
            }

            if (param.StudentId != Guid.Empty)
            {
                Criteria = CombineWithAnd(Criteria, sc => sc.StudentId == param.StudentId);
            }

            if (param.SchoolId.HasValue)
            {
                Criteria = CombineWithAnd(Criteria, sc => sc.Class.SchoolId == param.SchoolId.Value);
            }
